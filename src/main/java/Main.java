...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        switch (menuItems){
            case "1":
                contactService = new InMemoryContactsService();
                break;
            case "2":
                contactService = new JsonRealisationFileContactService
                        (new File("ContactsBook.json"));
                break;
            case "3":
                contactService = new XMLRealisationFileContactService
                        (new File("ContactsBook.xml"));
                break;
            case "4":
                contactService = new CSVRealisationFileContactService
                        (new File("ContactsBook.csv"));
                break;
            case "5":
                contactService = new BytesRealisationFileContactService
                        (new File("ContactsBook.obj"));
                break;
            case "6":
                contactService = new ServerContactsService();
                break;
            default:
